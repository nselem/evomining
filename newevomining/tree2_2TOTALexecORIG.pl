...
#-----------------------indexado de NP-----------------------------------
open (NP,"$apacheCGIpath/NPDB/$np_db")or die $!;
while(<NP>){
 chomp;
 if($_ =~ />/){
  $_ =~ s/>//;
...
open (LABELS, "tree/$numFile.labels")or die $!;
open (OUTLABELS, ">tree/$numFile.mapp")or die $!;
...
system "$newickPATH/nw_rename -l tree/$numFile.tree tree/$numFile.mapp |$newickPATH/nw_display -w 10600 -sr  -S -v 100 -i 'font-size:xx-small' -b 'opacity:0' -i 'visibility:hidden' -c tree/$numFile.map2 -o tree/ornament2.$numFile ->tree/$numFile.pp.svg";
...
<IFRAME SRC="/cgi-bin/newevomining/sendtoDrawContext.pl?$outdirANDfile" name="iframe_abajo" WIDTH=810 HEIGHT=350 FRAMEBORDER=0 >  </IFRAME>
