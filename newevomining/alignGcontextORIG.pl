...
foreach my $c (@dat){
    ...
    ###########weekend###############  
     system "muscle -in $outdir/blast/$c.concat.fasta -out $outdir/blast/$c.aln";
...
print LOG "antes de gblocks\n";
 #-------------------------------------------------------------------------------------------------
   system "whoami>quien";
  ###########weekend###############
  print LOG "------Gblocks ../$cc\gap.aln.out -b4=5 -b5=H -b3=10>LOG$cc.GBLOCks\n";
    system "/opt/Gblocks_0.91b/Gblocks ../$cc\gap.aln.out -b4=5 -b5=H -b3=10>LOG$cc.GBLOCks" or die$!;
  #------------------------------------------------------------------------------
...
#system "perl /home/evoMining/newevomining/converter.pl ../$cc\gap.aln.out-gb2 >$cc.stockLOG";
...
foreach my $cc (@dat){
 ...
 chdir "$outdir/blast/seqf/";
 ###########weekend###############  
 system "quicktree -in a -out t -boot 10000  $outdir/blast/seqf/$cc.stock > $outdir/blast/seqf/tree/$cc.tree";
 #########weekend######### 
 system "$newickPATH/nw_distance -n $outdir/blast/seqf/tree/$cc.tree >$outdir/blast/seqf/distance.$cc";
 $dist="distance.$cc";
 ...
 #########weekend######### 
 system "$newickPATH/nw_display -w 5600 -sr -S -v 100 -l 'font-size:x-small' -b 'visibility:hidden' -o $outdir/blast/seqf/tree/ornament.$cc $outdir/blast/seqf/tree/$cc.tree >$outdir/blast/seqf/tree/$cc.p.svg";
...
print qq |<div><td >tree</td></div>|;
 open(FF, "$outdir/hash.log"); ###aqui hay que modificar
 while ($xef = <FF>){
     chomp($xef);
     @array =split("---",$xef);
    ...
    foreach my $xc (@dat){
        if($array2[0] eq $xc){
          ...
          open(XT, "$outdir/blast/seqf/$onlyD") or die $onlyD;
...
sub findDistance2 {
 ...
 print SI "inicio $dis, $numerin\n";
  ...
  #########weekend######### 
   ...
   #########weekend#########  
   system "$newickPATH/nw_clade -c 1 $outdir/blast/seqf/tree/$numerin.tree $np |$newickPATH/nw_distance -n ->>$outdir/blast/seqf/distance.$numerin.only1";
    #print NPPS "$numerin.tree---$np\n";
   #########weekend######### 
    system "echo ==== >>$outdir/blast/seqf/distance.$numerin.only1";
  }
 #########weekend######### 
  close SI;
   $seccion=0;
   $colorSeccion=0;
 #########weekend#########
   open (NPPS,">$outdir/$numerin.NPPS.log") or die $!;
 #########weekend######### 
  open (DOUT, ">$outdir/blast/seqf/distance.$numerin.only")or die $!;
  open (D, "$outdir/blast/seqf/distance.$numerin.only1")or die $!;
  #########weekend#########
    open (ORNAMENT, ">$outdir/blast/seqf/tree/ornament.$numerin")or die $!;
  #########weekend#########
  ...
  close D;
  $cont=0; 
  $nomee="$outdir/blast/seqf/distance.$numerin.only.cssmap";
  #########weekend#########
  open (FDOUT, ">$nomee");
